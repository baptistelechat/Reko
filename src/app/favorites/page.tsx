...
import { ArrowLeft, Trash2, Star, Calendar, Film, Tv, Search, Heart } from 'lucide-react';
...
export default function FavoritesPage() {
  ...
  const [sortBy, setSortBy] = useState<'added' | 'rating' | 'title' | 'date'>('added');
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="mb-8 space-y-4">
          <div className="flex flex-col md:flex-row gap-4">
            ...
            <select
              value={sortBy}
              onChange={(e) => setSortBy(e.target.value as 'added' | 'rating' | 'title' | 'date')}
          ...
          <div className="flex gap-4 text-sm text-gray-600">
            <span>
              {filteredAndSortedFavorites.filter(item => item.type === 'movie').length} films
            </span>
            <span>
              {filteredAndSortedFavorites.filter(item => item.type === 'tv').length} séries
            </span>
            <span>
              Note moyenne: {
                filteredAndSortedFavorites.length > 0
                  ? (filteredAndSortedFavorites.reduce((acc, item) => acc + item.vote_average, 0) / filteredAndSortedFavorites.length / 2).toFixed(1)
                  : '0'
              }/5
            </span>
          </div>
