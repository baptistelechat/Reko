...
import { ArrowLeft, RefreshCw, Star, Calendar, Heart, Plus, Eye } from 'lucide-react';
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-reko-primary/5 to-reko-secondary/5 flex items-center justify-center">
        <div className="text-center space-y-4">
          ...
          <p className="text-lg font-medium text-gray-700">Recherche de recommandations...</p>
          <p className="text-sm text-gray-500">Nous analysons vos préférences</p>
        </div>
      </div>
    );
  }
      ...
      <div className="min-h-screen bg-gradient-to-br from-reko-primary/5 to-reko-secondary/5 flex items-center justify-center">
        <div className="text-center space-y-4 max-w-md">
          ...
          <div className="space-x-4">
            <Button onClick={handleRefresh} className="bg-reko-primary hover:bg-reko-primary/90">
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          <div className="text-center">
            <h1 className="text-2xl font-bold text-gray-900">Vos recommandations</h1>
            <div className="flex items-center justify-center gap-2 mt-1">
              <Badge variant="secondary">{getMoodLabel(preferences.mood!)}</Badge>
              <Badge variant="secondary">{getContentTypeLabel(preferences.contentType!)}</Badge>
            </div>
          </div>
        ...
        {recommendations.length === 0 ? (
          <div className="text-center py-12">
            ...
            <h2 className="text-xl font-semibold text-gray-700 mb-2">Aucune recommandation trouvée</h2>
            <p className="text-gray-500 mb-6">Essayez avec d'autres préférences</p>
            <Button onClick={() => router.push('/explore')} className="bg-reko-primary hover:bg-reko-primary/90">
              Nouvelle recherche
            </Button>
          </div>
        ) : (
                      ...
                      <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center">
                        <div className="flex space-x-2">
                          <Button
                            ...
                            disabled={isInWatchlist(content.id, 'title' in content ? 'movie' : 'tv')}
                            className="bg-white/90 hover:bg-white text-gray-900"
                          >
                            {isInWatchlist(content.id, 'title' in content ? 'movie' : 'tv') ? <Eye size={16} /> : <Plus size={16} />}
                          </Button>
                          <Button
                            size="sm"
                            variant="secondary"
                            onClick={() => handleAddToFavorites(content)}
                            disabled={isInFavorites(content.id, 'title' in content ? 'movie' : 'tv')}
                            className="bg-white/90 hover:bg-white text-gray-900"
                          >
                            <Heart 
                              size={16} 
                              className={isInFavorites(content.id, 'title' in content ? 'movie' : 'tv') ? 'fill-red-500 text-red-500' : ''} 
                            />
                          </Button>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center justify-between text-xs text-gray-500">
                        ...
                        {content.genre_ids && content.genre_ids.length > 0 && (
                          <Badge variant="outline" className="text-xs">
                            {content.genre_ids.length} genre{content.genre_ids.length > 1 ? 's' : ''}
                          </Badge>
                        )}
                      </div>
